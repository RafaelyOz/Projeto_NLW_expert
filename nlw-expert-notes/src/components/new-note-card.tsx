...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
    ...
    function handleStartRecording() {
        ...
        const isSpeechRecognitiAPIAvaiable = 'SpeechRecognition' in window
        || 'webkitSpeechRecognition' in window

        if(!isSpeechRecognitiAPIAvaiable){
            alert('Infelizmente seu não suporta a API de gravação!')
            return
        }
    ...
    return (
        <Dialog.Root>
            <Dialog.Trigger className="rounded-md flex flex-col bg-slate-700 text-left p-5 gap-y-3 outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
                ...
                <p className="text-sm leading-6 text-slate-400">Grave uma nota em áudio que será convertida para texto</p>
            </Dialog.Trigger>
            <Dialog.Portal>
                <Dialog.Overlay className="fixed inset-0 bg-black/60" />
                <Dialog.Content className="fixed overflow-hidden  insert-0 md:insert-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] md:w-full h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
                    ...
                    <form className='flex-1 flex flex-col'>
                        ...
                        {isRecording ? (
                            <button type='button' onClick={() =>{ setIsRecording(false); handleStopRecording()}} className='w-full flex items-center justify-center gap-2 bg-slate-900 py-4 text-center text-sm text-slate-300 outline-none font-medium hover:text-slate-100'>
